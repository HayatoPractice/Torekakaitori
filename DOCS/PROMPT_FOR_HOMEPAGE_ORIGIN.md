...
### 【設計判断ガイドライン】今回の作業で必ず守ること

以下はアプリ作成原本での改良作業を通じて確立した判断基準です。
このホームページ作成原本への作業でも同じ基準を適用してください。

---

#### ■ 一貫して守る設計原則

**① DOCS_INDEX.md を唯一のナビゲーション層にする**
ファイル間の「→ XXXを参照」という関係はすべて DOCS_INDEX.md のタスク別早見表が担う。
個別ファイルに同じ関係を書くのは重複であり、トークンコストが増えるだけ。やってはいけない。

**② 追加・注記は自律実行。削除・大規模書き換えは事前確認**
- 追加・ポインタ化・注記追加 → 影響が限定的なので自律実行してよい
- ファイルの削除・ファイル名変更・構造の大幅変更 → 必ず事前に確認を取ること

**③ 指示の前提条件がこのフォルダの構造と合わない場合は停止・報告**
「〇〇ファイルに追記してください」という指示に対し、そのファイルが存在しない場合は
勝手に作らず、「存在しません。どうしますか？」と確認すること。

**④ 元の良い部分を残しながら改良する（温故知新）**
既存ファイルのセクション・文体・構成を変えず、追記・ポインタ化・注記追加を優先する。
変更前と変更後で「新しく追加された情報があるか」を必ず確認すること。

**⑤ 実装後も「本当に必要か」を問い直す**
実装した後でも「これは既存の仕組みで解決できていないか」を検証し、
重複であれば提案して差し戻す。

---

#### ■ やってはいけないパターン（アプリ作成原本で実際に差し戻したもの）

以下は一見良さそうに見えるが、実際には改悪となるパターンです。

| やってしまいがちな変更 | なぜ差し戻したか |
|---|---|
| 各ファイルの教訓・ガイドに「■ 関連参照：→ XXXを参照」を追記 | DOCS_INDEX.md が既に担っている関係の重複。新情報ゼロ・トークン増のみ |
| CLAUDE.md に「設計時はこのファイルを読め」と特定ファイルを明示 | 明確なファイル指定で柔軟性が失われる。DOCS_INDEX.md の早見表が柔軟に担うべき役割 |
| 各ファイルに他ファイルへのポインタを分散して追記 | ナビゲーションが複数箇所に分散し、DOCS_INDEX.md の一元管理が崩れる |

**例外として残してよいもの：**
- 「このファイルは廃止されました → YYYに移動済み」という移行履歴の記録
- 曖昧な表現を具体的なID（例：ML-007）に明示化する変更

---

#### ■ SKIP_DOCS 管理（sync スクリプトの配布除外リスト）

同期スクリプト（sync_to_sites.py 等）内の `SKIP_DOCS` に追加すべきファイル：
- 各サイトフォルダ固有のデータファイル（PROJECT_STATE.md 等）
- 原本運用専用のファイル（このプロンプトファイル自体など）

**判断基準：** 「各サイトフォルダで実際に使うか？」→ Yes なら同期、No なら SKIP_DOCS に追加。

新しいファイルを DOCS/ に追加したとき、毎回この判断を行うこと。

---

PROMPT_FOR_HOMEPAGE_ORIGIN.md v2.0 — 作業ガイドライン（設計判断基準）を追記
