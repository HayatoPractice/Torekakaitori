...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
▌プロジェクト運用コマンド（起動・同期・終了）

手動でのファイル読み込みや指示を避け、以下のコマンドを使用してください。
- `/project:start` : セッション開始・現状把握
- `/project:new`   : 新規プロジェクトの初期化
- `/project:kickoff`: [NEW] 対話形式での要件定義とREQUIREMENTS_LOG初期化
- `/project:sync`  : 共通ファイルの全サイト同期
- `/project:audit` : [NEW] リリース前の自動品質・型チェック監査
- `/project:compact`: 長期ログの圧縮と教訓退避
...
▌セッション開始時のモード（冒頭で必ず確認する）
  ...
  モード未宣言の場合：
  MINUTES.mdのlast_completedを確認して自動判定する。
  判定できない場合は「新規作成ですか？途中再開ですか？」と確認する。

  **【超重要：新規作成時のAIヒアリング】**
  現在のフェーズが「初期セットアップ完了・要件定義待ち」と判定された場合、
  AIは**自律的にヒアリングモード**に入ること。
  ユーザーに対して「どのようなアプリを作成しますか？（目的、主な機能など）」と質問し、
  対話を通して要件を引き出し、`DOCS/REQUIREMENTS_LOG.md` を自動作成・記述すること。
...
DOCS/MINUTES.md    前回セッションの状態・引き継ぎ事項

【第2層-B：実装・設計時に読む（必要な時だけ）】
DOCS/MASTER_LESSONS.md          技術スタック標準・Kill基準・AI選定基準・収益化方針
...
□ REQUIREMENTS_LOG.md  → 新規作成（AIによるヒアリングを通じて自動生成）
□ MINUTES.md    → 新規作成（初期セットアップ完了状態でスタート）
□ APP_STRUCTURE_REFERENCE.md → プロジェクト構造に合わせて作成
□ AUTO_SYNC_GUIDE.md   → 同期設計が必要な場合のみ作成

【アーカイブする】
□ 前プロジェクトのBDR → ARCHIVE/[PJ名]_BDR/
□ 前プロジェクトのMINUTES.md → ARCHIVE/[PJ名]_CONTEXT/
...
STEP 3｜DOCS/MINUTES.md を読み、現在地・開発情報・タスクボードを把握する
STEP 4｜DOCS/MINUTES.md を読み、前回セッションの状態を引き継ぐ
...
前回の最終タスク：[PROJECT_STATEより / 新規の場合「なし」]
...
SERVICE_ORG_CORE.md       → SERVICE_ORG_PHASE.md / MINUTES.md
SERVICE_ORG_PHASE.md      → MINUTES.md / REQUIREMENTS_LOG.md
MINUTES.md                → なし（他ファイルがここを参照するだけ）
REQUIREMENTS_LOG.md       → MINUTES.md
MINUTES.md         → 全チームが参照（セッション開始時の状態把握）
MASTER_LESSONS.md         → 全チームが参照（参考教訓の提示）
MASTER_LESSONS.md         → 技術選定・kill判断・価格設定・AI選定時に参照
