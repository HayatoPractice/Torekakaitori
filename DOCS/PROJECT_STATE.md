...
- 2026-09-03、画像1枚のみでAI解析すると504（サーバー側タイムアウト）が出る不具合を調査。
  スマホカメラの未圧縮写真（数MB・4000px超）だとアップロード・AI解析に時間がかかりすぎるのが
  主因と推定し、`src/lib/file.ts`の`prepareImageForUpload`でアップロード前にcanvasで
  1600px・JPEG品質0.85まで圧縮するようにした（800KB以下の画像はそのまま送る）。
  サーバー側にも10MB超の画像を即座に弾く保険を追加。原因の完全な再現はできていない
  （ローカルでは1.8MB・4032x3024の画像でもGemini呼び出し自体は17〜19秒で成功しており、
  実際に504を引き起こした条件—回線速度・画像サイズ等—は特定できていない）ため、
  実機での再発有無を確認してほしい旨をユーザーに伝えてある
- 2026-09-03、AI解析でカード/BOXの価格情報が1件も見つからなかった場合（無関係な画像・
  雑談テキスト等）、その投稿はDBに残さず取り消し、「登録していません」とはっきり案内する
  ようにした（`noContentFound`フラグ。URL単体投稿の既存の案内とは区別して扱う）
- 2026-09-03、DOCSに保存した「ポケモンカード発売一覧」（公式サイト・複数の専門メディアで
  ファクトチェック済み、2022〜2026年・約100商品・再販有無の調査結果を含む）を`products`
  テーブルへ一括登録した。商品名・再販履歴メモ（`resale_notes`列）はホーム画面の商品一覧から
  「編集」ボタンで後から修正できる（`PATCH /api/products/[id]`）。一覧は元々検索欄を
  備えていたため、投稿を1件も送らなくても登録済み商品が検索・閲覧できる状態になっている。
  既存の実データ（`シュリンク有/無`付きの商品名等）とは名前が完全一致しないため重複登録は
  発生していないことを確認済み（統合が必要なら既存の「表記ゆれの統合」機能を使う）
